 ...
 * @author ookawara
 */
public class MG0220EditDisplayDialogCtrl extends PanelAndDialogCtrlBase {

	/** 管理マスタダイアログ */
	protected MG0220EditDisplayDialog dialog;
	 ...
	 * @param titleid
	 */
	MG0220EditDisplayDialogCtrl(Frame parent, String titleid) {
		// 管理マスタダイアログの初期化
		dialog = createDialog(parent, titleid);

		knrName = CompanyControlHelper220.getInstance(this.getLoginUserCompanyCode()).getKnrName5();
	 ...
	 * @return ダイアログ
	 */
	protected MG0220EditDisplayDialog createDialog(Frame parent, String titleid) {
		return new MG0220EditDisplayDialog(parent, true, this, titleid);
	}
...
class CompanyControlHelper220 extends TAppletClientBase {

	private static Map instances = Collections.synchronizedMap(new HashMap());

	/**
	 * @param companyCode
	 * @return CompanyControlHelper220
	 */
	public static CompanyControlHelper220 getInstance(String companyCode) {
		if (!instances.containsKey(companyCode)) {
			CompanyControlHelper220 instance = new CompanyControlHelper220(companyCode);
			instances.put(companyCode, instance);
		}
		return (CompanyControlHelper220) instances.get(companyCode);
	}
	...
	private CompanyControlHelper220(String companyCode) {
