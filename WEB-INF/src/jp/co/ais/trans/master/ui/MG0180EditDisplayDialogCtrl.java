 ...
 * @author ookawara
 */
public class MG0180EditDisplayDialogCtrl extends PanelAndDialogCtrlBase {

	/** 管理マスタダイアログ */
	protected MG0180EditDisplayDialog dialog;
	 ...
	 * @param titleid
	 */
	MG0180EditDisplayDialogCtrl(Frame parent, String titleid) {
		// 管理マスタダイアログの初期化
		dialog = createDialog(parent, titleid);
		// タイトルを設定する

		knrName = CompanyControlHelper.getInstance(this.getLoginUserCompanyCode()).getKnrName1();
	 ...
	 * @return ダイアログ
	 */
	protected MG0180EditDisplayDialog createDialog(Frame parent, String titleid) {
		return new MG0180EditDisplayDialog(parent, true, this, titleid);
	}
	 ...
	 */
	public void show(boolean isEnabledknrCode) {
		// 画面を表示する
	...
	protected void setValues(Map map) {
		...
		if (isUpdate) {
			dialog.ctrlManagementName.getField().requestFocus();
		}
	}
	...
	protected boolean checkCode(String code) throws IOException {
		// 管理コード未入力
...
class CompanyControlHelper extends TAppletClientBase {

	private static Map instances = Collections.synchronizedMap(new HashMap());

	/**
	 * @param companyCode
	 * @return CompanyControlHelper
	 */
	public static CompanyControlHelper getInstance(String companyCode) {
		if (!instances.containsKey(companyCode)) {
			CompanyControlHelper instance = new CompanyControlHelper(companyCode);
			instances.put(companyCode, instance);
		}
		return (CompanyControlHelper) instances.get(companyCode);
	}
	...
	private CompanyControlHelper(String companyCode) {
