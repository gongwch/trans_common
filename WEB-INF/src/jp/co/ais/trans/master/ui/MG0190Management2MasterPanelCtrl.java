 ...
 * @author ookawara
 */
public class MG0190Management2MasterPanelCtrl extends TPanelCtrlBase {

	/** プログラムID */
	protected static final String PROGRAM_ID = "MG0190";
	...
	protected String getServletName() {
		return "MG0190Management2MasterServlet";
	}
	...
	/** パネル */
	protected MG0190Management2MasterPanel panel;
	 ...
	 * コンストラクタ.
	 */
	public MG0190Management2MasterPanelCtrl() {

		knrName2 = CompanyControlHelper190.getInstance(this.getLoginUserCompanyCode()).getKnrName2();
		columnName = new String[] { "C00596", knrName2 + this.getWord("C00174"), knrName2 + this.getWord("C00518"),
				knrName2 + this.getWord("C00548"), knrName2 + this.getWord("C00160"), "C00055", "C00261" };
		try {
			// 一覧画面の初期化
			panel = new MG0190Management2MasterPanel(this);
		} catch (Exception e) {
			// 正常に処理されませんでした
			errorHandler(panel.getParentFrame(), e, "E00010");
		}
		panel.ctrlBeginManagement.getButton().setText(knrName2);
		panel.ctrlEndManagement.getButton().setText(knrName2);
		// 画面初期化
	...
	void init() {
		...
		ref1.setTitleID(knrName2 + this.getWord("C00500"));
		ref1.setPrefixID(knrName2);
		...
		ref2.setTitleID(knrName2 + this.getWord("C00500"));
		ref2.setPrefixID(knrName2);
	...
	void insert() {
		try {
			// 編集画面の初期化
			MG0190EditDisplayDialogCtrl dialog = new MG0190EditDisplayDialogCtrl(panel.getParentFrame(), "C01698");
			// 編集画面の表示
			dialog.show(true);
			// 編集画面が開けていました
	...
	void update() {
		try {
			MG0190EditDisplayDialogCtrl dialog = new MG0190EditDisplayDialogCtrl(panel.getParentFrame(), "C00977");
			// 当前行を取得する
	...
	public void copy() {
		try {
			MG0190EditDisplayDialogCtrl dialog = new MG0190EditDisplayDialogCtrl(panel.getParentFrame(), "C01738");
			// 当前行を取得する
			int nomRow = panel.ssJournal.getCurrentRow();
			// データ集を取得する
			TableDataModel model = panel.ssJournal.getDataSource();
	 ...
	 * @throws IOException
	 */
	private void modifySpreadRow(String krnCode, int updKbn) throws TRequestException, IOException {
	...
	boolean find() {
		...
		try {
			...
			if (Util.isSmallerThen(strBegin, strEnd) == false) {
				...
				showMessage(panel, "W00036", knrName2);
				return false;
			}
