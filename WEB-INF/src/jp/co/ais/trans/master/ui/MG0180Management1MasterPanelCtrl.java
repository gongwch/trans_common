 ...
 * @author ookawara
 */
public class MG0180Management1MasterPanelCtrl extends TPanelCtrlBase {

	/** プログラムID */
	private static final String PROGRAM_ID = "MG0180";
	...
	/** パネル */
	protected MG0180Management1MasterPanel panel;

	/** カラム名 */
	public String[] columnName;

	protected String knrName1;

	protected REFDialogCtrl ref1;

	protected REFDialogCtrl ref2;

	MG0180EditDisplayDialogCtrl dialog;
	...
	protected String getServletName() {
		return "MG0180Management1MasterServlet";
	}

	/**
	 * コンストラクタ.
	 */
	public MG0180Management1MasterPanelCtrl() {

		knrName1 = CompanyControlHelper.getInstance(this.getLoginUserCompanyCode()).getKnrName1();

		columnName = new String[] { "C00596", knrName1 + this.getWord("C00174"), knrName1 + this.getWord("C00518"),
				knrName1 + this.getWord("C00548"), knrName1 + this.getWord("C00160"), "C00055", "C00261" };

		try {
			// 一覧画面の初期化
			panel = new MG0180Management1MasterPanel(this);
		} catch (Exception e) {
			// 正常に処理されませんでした
		...
		panel.ctrlBeginManagement.getButton().setText(knrName1);
		panel.ctrlEndManagement.getButton().setText(knrName1);
	...
	protected void init() {
		...
		ref1.setTitleID(knrName1 + this.getWord("C00500"));
		ref1.setPrefixID(knrName1);
		...
		ref2.setTitleID(knrName1 + this.getWord("C00500"));
		ref2.setPrefixID(knrName1);
	...
	protected void createEditDisplayDialog(String titleId) {
		dialog = new MG0180EditDisplayDialogCtrl(panel.getParentFrame(), titleId);
	}
	 ...
	 * @throws IOException
	 */
	protected void modifySpreadRow(String knrCode, int updKbn) throws TRequestException, IOException {
		Vector<Vector> cells = setModifyCell(knrCode, updKbn);
	...
	protected boolean find() {
		try {
			...
			if (Util.isSmallerThen(strBegin, strEnd) == false) {
				...
				showMessage(panel, "W00036", knrName1);
				return false;
			}
			// 表示データの取得
			return reflesh();
