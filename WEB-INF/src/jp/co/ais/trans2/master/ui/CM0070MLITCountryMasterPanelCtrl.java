...
public class CM0070MLITCountryMasterPanelCtrl extends TController {
	...
	protected void btnExcel_Click() {
		...
		try {
			...
			printer.preview(data, getWord("YJ Country Master") + ".xls"); // YJ Countryマスタ.xls
	...
	protected boolean isInputRight() throws Exception {
		...
		if (Util.isNullOrEmpty(editView.ctrlItem.getCode())) {
			// エラーメッセージ出力
			showMessage(editView, "I00002", "Region Code"); // 国コード
		...
		if (Util.isNullOrEmpty(editView.ctrlCode.getValue())) {
			// エラーメッセージ出力
			showMessage(editView, "I00002", "Country Code"); // 地域コード
		...
		if (Mode.NEW == mode || Mode.COPY == mode) {
			...
			if (list != null && !list.isEmpty()) {
				showMessage(editView, "I00055", "Country Code"); // 地域コード
	...
	protected void reflectEditBean() {
		...
		editBean.setREGION_CODE(editView.ctrlItem.getCode()); // 国コード
		editBean.setREGION_NAME(editView.ctrlItem.getNames()); // 国名称
		editBean.setCOUNTRY_CODE(editView.ctrlCode.getValue()); // 地域コード
		editBean.setCOUNTRY_NAME(editView.ctrlName.getValue()); // 地域名称
	...
	protected void initEditView(Mode editMode, YJRegion bean) {
		...
		switch (editMode) {
			case NEW:
				// 新規
				editView.setTitle(getWord("Country") + " " + getWord("C01698")); // 新規画面
			...
			case MODIFY:
				// 編集
				editView.setTitle(getWord("Country") + " " + getWord("C00977")); // 編集画面
			...
			case COPY:
				// 複写
				editView.setTitle(getWord("Country") + " " + getWord("C01738")); // 複写画面
	...
	protected void setEditDialog(YJRegion bean) {
		...
		editView.ctrlItem.setCode(bean.getREGION_CODE()); // 国コード
		editView.ctrlItem.setNames(bean.getREGION_NAME()); // 国名称
		editView.ctrlCode.setValue(bean.getCOUNTRY_CODE()); // 地域コード
		editView.ctrlName.setValue(bean.getCOUNTRY_NAME()); // 地域名称
	...
	protected List<String> getRowData(YJRegion bean) {
		...
		list.add(bean.getREGION_CODE()); // 国コード
		list.add(bean.getREGION_NAME()); // 国名称
		list.add(bean.getCOUNTRY_CODE()); // 地域コード
		list.add(bean.getCOUNTRY_NAME()); // 地域名称
