 ...
 * 承認権限ロールマスタの指示画面
 */
public class MG0500AprvRoleMasterPanel extends TMainPanel {
	...
	/** 範囲検索コンポーネント */
	public TAprvRoleReferenceRange ctrlSearch;
	 ...
	 * @param company
	 */
	public MG0500AprvRoleMasterPanel(Company company) {
		super(company);
	}
	...
	public void initComponents() {
		...
		ctrlSearch = new TAprvRoleReferenceRange();
		ctrlOutputTermEnd = new TCheckBox();

		tbl = new TTable();
		tbl.addColumn(SC.bean, "BEAN", -1);
		tbl.addColumn(SC.roleCode, "C11154", 100); // ロールコード
		tbl.addColumn(SC.roleName, "C11155", 200); // ロール名称
		tbl.addColumn(SC.roleNames, "C11156", 200); // ロール略称
		tbl.addColumn(SC.roleNamek, "C11157", 200); // ロール検索名称
