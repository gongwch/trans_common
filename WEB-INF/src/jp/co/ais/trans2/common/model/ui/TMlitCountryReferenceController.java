...
/**
 * 輸送実績地域マスタ検索コンポーネントのコントローラ
 * 
 * @author AIS
 */
public class TMlitCountryReferenceController extends TReferenceController {
	 ...
	 */
	public TMlitCountryReferenceController(TReference field) {
		super(field);
	}
	...
	public boolean code_Verify(JComponent comp) {
		...
		if (entity != null) {
			...
			field.name.setText(getEntity().getCOUNTRY_NAME());
			return true;
		}
	...
	public void btnSearch_Click() {
		...
		try {
			...
			// コード前方一致
			condition_.setCountryCodeLike(dialog.code.getText());
			// 略称あいまい
			condition_.setCountryNameLike(dialog.names.getText());
			...
			for (YJRegion bean : list) {
				dialog.tbl.addRow(new Object[] { bean, bean.getCOUNTRY_CODE(), bean.getCOUNTRY_NAME() });
			}
	...
	public void btnSettle_Click() {
		...
		try {
			...
			if (entity != null) {
				field.code.setText(getEntity().getCOUNTRY_CODE());
				field.name.setText(getEntity().getCOUNTRY_NAME());
	...
	protected YJRegion getInputtedEntity() {
		...
		try {
			...
			searchCondition.setCountryCode(field.getCode());
	...
	/**
	 * 検索条件に該当する輸送実績地域マスタリストを返す
	 * 
	 * @param condition_ 検索条件
	 * @return 検索条件に該当する輸送実績地域マスタリスト
	 */
	protected List<YJRegion> getList(YJRegionSearchCondition condition_) {
		try {
			List<YJRegion> list = (List<YJRegion>) request(getModelClass(), "getCountry", condition_);
			return list;
	...
	public String getCodeCaption() {
		return getWord("Country Code"); // Country Code
	}
	...
	public String getNameCaption() {
		return getWord("Country Name"); // Country Name
	}
	...
	public void setEntity(YJRegion bean) {
		...
		field.code.setText(bean.getCOUNTRY_CODE());
		field.name.setText(bean.getCOUNTRY_NAME());
		entity = bean;
	}
	...
	public Object getUnspecifiedEntity() {
		...
		bean.setCOUNTRY_CODE(field.code.getText());
		bean.setCOUNTRY_NAME(field.name.getText());
		return bean;
	}
