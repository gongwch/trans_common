...
/**
 * 輸送実績サブアイテムマスタ検索コンポーネントのコントローラ
 * 
 * @author AIS
 */
public class TMlitSubItemReferenceController extends TReferenceController {
	 ...
	 */
	public TMlitSubItemReferenceController(TReference field) {
		super(field);
	}
	...
	public boolean code_Verify(JComponent comp) {
		...
		if (entity != null) {
			...
			field.name.setText(getEntity().getITEM_SUB_NAME());
			return true;
		}
	...
	public void btnSearch_Click() {
		...
		try {
			...
			// コード前方一致
			condition_.setSubItemCodeLike(dialog.code.getText());
			// 略称あいまい
			condition_.setSubItemNameLike(dialog.names.getText());
			...
			for (YJItem bean : list) {
				dialog.tbl.addRow(new Object[] { bean, bean.getITEM_SUB_CODE(), bean.getITEM_SUB_NAME() });
			}
	...
	public void btnSettle_Click() {
		...
		try {
			...
			if (entity != null) {
				field.code.setText(getEntity().getITEM_SUB_CODE());
				field.name.setText(getEntity().getITEM_SUB_NAME());
	...
	protected YJItem getInputtedEntity() {
		...
		try {
			...
			searchCondition.setSubItemCode(field.getCode());
	...
	/**
	 * 検索条件に該当する輸送実績サブアイテムマスタリストを返す
	 * 
	 * @param condition_ 検索条件
	 * @return 検索条件に該当する輸送実績サブアイテムマスタリスト
	 */
	protected List<YJItem> getList(YJItemSearchCondition condition_) {
		try {
			List<YJItem> list = (List<YJItem>) request(getModelClass(), "getSubItems", condition_);
			return list;
	...
	/**
	 * 選択された輸送実績サブアイテムマスタを返す
	 * 
	 * @return 選択された輸送実績サブアイテムマスタ
	 */
	...
	public String getCodeCaption() {
		return getWord("Sub Item Code"); // Item Code
	}
	...
	public String getNameCaption() {
		return getWord("Sub Item Name"); // Item Name
	}
	...
	public void setEntity(YJItem bean) {
		...
		field.code.setText(bean.getITEM_SUB_CODE());
		field.name.setText(bean.getITEM_SUB_NAME());
		entity = bean;
	}
	...
	public Object getUnspecifiedEntity() {
		...
		bean.setITEM_SUB_CODE(field.code.getText());
		bean.setITEM_SUB_NAME(field.name.getText());
		return bean;
	}
