...
/**
 * 会社間付替処理<グループ会計版 取引通貨が異なる場合>
 * 
 * @author AIS YF.CONG
 */
public class GroupCompanyTransferDifferentCurrency extends TModel {
	...
	public List<Slip> transfer(final Slip slip) throws TException {
		...
		for (SWK_DTL dtl : dtlList) {
			...
			if (baseCompanyCode.equals(kKaiCode)) {
				...
				// 付替先明細(付替) 2(他社)
				SWK_DTL tukeautodtl = makeChildAutoDtl(kKaiCode, tukemotodtl, dtl, ++gyoNo, baseCompanyCode);
				// 基軸通貨JPY
				String baseCurrencyCode = getCompany().getAccountConfig().getKeyCurrency().getCode();

				// 子会社情報
				Company company = getCompany(kKaiCode);

				// 子会社基軸通貨コード
				String keyCurrencyCode = company.getAccountConfig().getKeyCurrency().getCode();

				if (baseCurrencyCode.equals(keyCurrencyCode)) {

					// 親会社の基軸通貨＝子会社の基軸通貨の場合
					if (dtl.getCurrency() != null) {
						tukemotodtl.setCurrency(dtl.getCurrency());
					} else {
						tukemotodtl.setSWK_CUR_CODE(dtl.getSWK_CUR_CODE());
					}
					tukemotodtl.setSWK_CUR_RATE(dtl.getSWK_CUR_RATE());
					tukemotodtl.setSWK_KIN(dtl.getSWK_KIN());
					tukemotodtl.setSWK_IN_KIN(dtl.getSWK_IN_KIN());
				}
				if (isBsUseKCompany) {
					tukemotodtl.setBsDetail(null);
	...
	protected SWK_DTL makeParentDtl(String kaiCode, SWK_DTL dtl, int gyoNo, int swkTukeKbn) throws TException {
		...
		// 科目設定フラグがOFFの場合、消費税設定不要
		if (item != null && item.getPreferedItem() != null
			&& !(item.getPreferedItem().isUsePurchaseTaxation() && item.getPreferedItem().isUseSalesTaxation())) {

			newDtl.setSWK_IN_KIN(dtl.getSWK_IN_KIN().add(DecimalUtil.avoidNull(dtl.getSWK_IN_ZEI_KIN())));
			newDtl.setSWK_KIN(dtl.getSWK_KIN().add(DecimalUtil.avoidNull(dtl.getSWK_ZEI_KIN())));
	...
	protected SWK_DTL makeChildDtl(String kaiCode, SWK_DTL dtl, int gyoNo, int tukeKbn) {
		...
		// 自動仕訳区分 ＝ 0:通常
		newDtl.setSWK_AUTO_KBN(SWK_DTL.AUTO_KBN.NOMAL);
		...
		// 機能通貨USD
		String baseFunctionCurrencyCode = getCompany().getAccountConfig().getFunctionalCurrency().getCode();
		...
		// 入力通貨コード
		String currencyCode = newDtl.getSWK_CUR_CODE();
		...
		// 入力金額
		BigDecimal foreignAmount = null;
		// 入力税額
		BigDecimal foreignZeiAmt = newDtl.getSWK_IN_ZEI_KIN();
		// 邦貨金額
		BigDecimal amount = null;
		// 邦貨税額
		BigDecimal zeiamt = newDtl.getSWK_ZEI_KIN();

		if (currencyCode.equals(keyCurrencyCode)) {
			// 子会社基軸通貨コード＝入力通貨コードの場合、レート＝入力レート
			rate = newDtl.getSWK_CUR_RATE();

		} else {
			if (AccountBook.IFRS.equals(newDtl.getAccountBook())) {
				// IFRS
				rate = getRate(kaiCode, baseFunctionCurrencyCode, newDtl);
			} else {
				// 自国
				rate = getRate(kaiCode, baseCurrencyCode, newDtl);
			}
		}

		// 入力金額
		// if (SWK_DTL.ZEI_KBN.TAX_IN == newDtl.getSWK_ZEI_KBN()) {
		// // →内税 付替仕訳作成元の邦貨金額－付替仕訳作成元の邦貨消費税額
		// foreignAmount = newDtl.getSWK_KIN().subtract(newDtl.getSWK_ZEI_KIN());
		// } else {
		// // →外税 付替仕訳作成元の邦貨金額
		// foreignAmount = newDtl.getSWK_KIN();
		// }
		// そのまま
		foreignAmount = newDtl.getSWK_KIN();
		...
		if (currencyCode.equals(keyCurrencyCode)) {
			...
			// 基軸通貨なら同値・同通貨をセット
			rate = BigDecimal.ONE;
			foreignAmount = newDtl.getSWK_IN_KIN();
			foreignZeiAmt = newDtl.getSWK_IN_ZEI_KIN();

			baseCurrencyCode = keyCurrencyCode;
			...
			// // →内税 付替仕訳作成元の邦貨金額－付替仕訳作成元の邦貨消費税額
			// // 入力金額 ＝ 邦貨金額－邦貨消費税額
			// amount = newDtl.getSWK_IN_KIN().subtract(newDtl.getSWK_IN_ZEI_KIN());
			// } else {
			// // →外税 付替仕訳作成元の邦貨金額
			// // 入力金額 ＝ 邦貨金額
			// amount = newDtl.getSWK_IN_KIN();
			// }
			// そのまま
			amount = newDtl.getSWK_IN_KIN();
			zeiamt = newDtl.getSWK_IN_ZEI_KIN();

		} else {
			// ２.入力伝票仕訳の通貨＜＞海外子会社の基軸通貨の場合
			amount = convertKeyAmount(foreignAmount, rate, company, currency);
			// ログイン基軸通貨と子会社基軸通貨が異なる場合は消費税計算
			if (!Util.equals(baseCurrencyCode, keyCurrencyCode)) {
				zeiamt = convertKeyAmount(foreignZeiAmt, rate, company, currency);
			}
		}

		// 通貨コード
		newDtl.setSWK_CUR_CODE(baseCurrencyCode);
		...
		newDtl.setSWK_IN_ZEI_KIN(DecimalUtil.isNullOrZero(foreignZeiAmt) ? null : foreignZeiAmt);

		// 邦貨金額
		newDtl.setSWK_KIN(amount);
		newDtl.setSWK_ZEI_KIN(DecimalUtil.isNullOrZero(zeiamt) ? null : zeiamt);
	 ...
	 * @throws TException
	 */
	protected SWK_DTL makeChildAutoDtl(String kaiCode, SWK_DTL dtl, SWK_DTL motoDtl, int gyoNo, String toCompanyCode)
		throws TException {
		// 会社間付替マスタから取得
		String fromCompany = kaiCode;
		...
		// 科目設定フラグがOFFの場合、消費税設定不要
		// 付替先の基軸通貨
		String baseCurrencyCode = getCompany(newDtl.getKAI_CODE()).getAccountConfig().getKeyCurrency().getCode();
		// 付替元の基軸通貨
		String toCurrencyCode = getCompany(toCompany).getAccountConfig().getKeyCurrency().getCode();

		boolean isBaseCur = Util.equals(baseCurrencyCode, toCurrencyCode);

		BigDecimal foreignAmount = null;
		BigDecimal amount = null;
		if (isBaseCur && Util.equals(dtl.getSWK_CUR_CODE(), motoDtl.getSWK_CUR_CODE())) {
			// 画面通貨と同じ
			foreignAmount = motoDtl.getSWK_IN_KIN().add(DecimalUtil.avoidNull(motoDtl.getSWK_IN_ZEI_KIN())); // 金額は画面値
			amount = motoDtl.getSWK_KIN().add(DecimalUtil.avoidNull(motoDtl.getSWK_ZEI_KIN())); // 金額は画面値
		} else if (isBaseCur && Util.equals(baseCurrencyCode, dtl.getSWK_CUR_CODE())) {
			// 基軸通貨と同じ
			foreignAmount = motoDtl.getSWK_KIN().add(DecimalUtil.avoidNull(motoDtl.getSWK_ZEI_KIN())); // 金額は画面値
			amount = motoDtl.getSWK_KIN().add(DecimalUtil.avoidNull(motoDtl.getSWK_ZEI_KIN())); // 金額は画面値
		} else {
			// そのまま
			foreignAmount = dtl.getSWK_IN_KIN().add(DecimalUtil.avoidNull(dtl.getSWK_IN_ZEI_KIN())); // 金額は画面値
			amount = dtl.getSWK_KIN().add(DecimalUtil.avoidNull(dtl.getSWK_ZEI_KIN())); // 金額は画面値
		}
		newDtl.setSWK_IN_KIN(foreignAmount); // 金額は画面値
		newDtl.setSWK_KIN(amount); // 金額は画面値
